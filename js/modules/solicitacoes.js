...
window.carregarSolicitacoes = async function (pagina = 1) {
    ...
    try {
        ...
        const { solicitacoes, total, totalPaginas } = await res.json()
        ...
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <div style="display:flex; gap:8px; align-items:center;">
            ...
            <button class="btn btn-sm btn-secondary" onclick="carregarSolicitacoes(${pagina + 1})" ${pagina >= totalPaginas ? 'disabled' : ''}>Próxima →</button>
          </div>
        </div>
      `
        }
...
let itensSC = []
let fornecedoresSC = []
let precosSC = {} // chave "itemIndex-fornecedorIndex" -> valor

function novoItemSC() {
    itensSC.push({ quantidade: '', unidade: '', descricao: '' })
}

function novoFornecedorSC() {
    fornecedoresSC.push({ nome: '', documento: '', telefone: '', prazoEntrega: '', condicoesPagto: '', observacoes: '', favorito: false })
}
...
window.abrirFormularioSolicitacao = async function (dadosExistentes = null) {
    ...
    if (dadosExistentes) {
        itensSC = dadosExistentes.itens.map(i => ({ quantidade: i.quantidade, unidade: i.unidade || '', descricao: i.descricao }))
        fornecedoresSC = dadosExistentes.fornecedores.map(f => ({
        ...
        dadosExistentes.itens.forEach((item, iIdx) => {
            dadosExistentes.fornecedores.forEach((forn, fIdx) => {
                ...
                if (preco) precosSC[`${iIdx}-${fIdx}`] = preco.valor
            })
        })
    } else {
        itensSC = []
...
function renderizarItensSC() {
    ...
    container.innerHTML = itensSC.map((item, i) => `
    <div style="display:grid; grid-template-columns: 100px 100px 1fr 40px; gap:8px; margin-bottom:8px;">
      <input type="number" class="form-control" placeholder="Qtd" value="${item.quantidade}" oninput="itensSC[${i}].quantidade = this.value">
      <input type="text" class="form-control" placeholder="Unid" value="${item.unidade}" oninput="itensSC[${i}].unidade = this.value">
      <input type="text" class="form-control" placeholder="Descrição" value="${item.descricao}" oninput="itensSC[${i}].descricao = this.value">
...
window.removerItemSC = function (i) {
    itensSC.splice(i, 1)
    // Remove preços associados a esse item, reindexando as chaves restantes
    const novosPrecos = {}
    Object.entries(precosSC).forEach(([chave, valor]) => {
        const [iIdx, fIdx] = chave.split('-').map(Number)
        if (iIdx === i) return
        const novoIdx = iIdx > i ? iIdx - 1 : iIdx
        novosPrecos[`${novoIdx}-${fIdx}`] = valor
    })
    precosSC = novosPrecos
    renderizarItensSC()
    renderizarMatrizPrecosSC()
}
...
function renderizarFornecedoresSC() {
    ...
    container.innerHTML = fornecedoresSC.map((f, i) => `
    <div style="border:1px solid #ddd; border-radius:6px; padding:12px; margin-bottom:8px;">
      <div style="display:grid; grid-template-columns: 2fr 1fr 1fr 40px; gap:8px; margin-bottom:8px;">
        <input type="text" class="form-control" placeholder="Nome *" value="${f.nome}" oninput="fornecedoresSC[${i}].nome = this.value">
        <input type="text" class="form-control" placeholder="CNPJ/CPF" value="${f.documento}" oninput="fornecedoresSC[${i}].documento = this.value">
        <input type="text" class="form-control" placeholder="Telefone" value="${f.telefone}" oninput="fornecedoresSC[${i}].telefone = this.value">
        <button class="btn btn-sm btn-danger" onclick="removerFornecedorSC(${i})">✕</button>
      </div>
      <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:8px;">
        <input type="text" class="form-control" placeholder="Prazo de entrega" value="${f.prazoEntrega}" oninput="fornecedoresSC[${i}].prazoEntrega = this.value">
        <input type="text" class="form-control" placeholder="Condições pagto" value="${f.condicoesPagto}" oninput="fornecedoresSC[${i}].condicoesPagto = this.value">
        <label style="display:flex; align-items:center; gap:6px; font-size:13px;">
          <input type="checkbox" ${f.favorito ? 'checked' : ''} onchange="fornecedoresSC[${i}].favorito = this.checked">
          Favorito
        </label>
      </div>
    </div>
...
function renderizarMatrizPrecosSC() {
    const container = document.getElementById('matriz-precos-sc')
    if (itensSC.length === 0 || fornecedoresSC.length === 0) {
    ...
    <table class="table-certificados">
      <thead>
        <tr>
          <th>Item</th>
          ${fornecedoresSC.map(f => `<th>${f.nome || '(sem nome)'}</th>`).join('')}
        </tr>
      </thead>
      <tbody>
        ${itensSC.map((item, iIdx) => `
          <tr>
            <td>${item.descricao || '(sem descrição)'}</td>
            ${fornecedoresSC.map((f, fIdx) => `
              <td>
                <input type="number" step="0.01" class="form-control form-control-sm"
                  value="${precosSC[`${iIdx}-${fIdx}`] ?? ''}"
                  oninput="precosSC['${iIdx}-${fIdx}'] = this.value">
              </td>
            `).join('')}
          </tr>
        `).join('')}
...
window.salvarSolicitacao = async function (id) {
    ...
    if (itensSC.length === 0 || itensSC.some(i => !i.descricao)) {
        alert('Todos os itens precisam de uma descrição!')
        return
    }
    if (fornecedoresSC.length === 0 || fornecedoresSC.some(f => !f.nome)) {
        alert('Todos os fornecedores precisam de um nome!')
        return
    }

    const precos = Object.entries(precosSC)
    ...
    const body = { empresaId, instrucoes, itens: itensSC, fornecedores: fornecedoresSC, precos }
    ...
    if (res.ok) {
        ...
        if (!id) {
            // Abre a assinatura do solicitante automaticamente, igual às OCs
          ...
          <thead>
            <tr>
              ...
              ${sc.fornecedores.map(f => `
                <th>
                  ${f.nome} ${f.favorito ? '⭐' : ''} ${f.escolhido ? '<span style="color:#198754;">✓ Escolhido</span>' : ''}
                </th>
              `).join('')}
            </tr>
          </thead>
