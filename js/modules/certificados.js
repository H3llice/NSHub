...
function renderCertificado(c) {
  ...
  return `
    <div style="margin-top:20px;">
      ...
      <a class="btn btn-secondary" style="margin-left:8px;" href="${API}/certificados/${c.id}/pdf?token=${encodeURIComponent(tokenAtual)}" target="_blank">📄 PDF</a>
      <h3 style="margin:20px 0;">
        ${emitido ? `Certificado ${c.numero}/${c.ano}` : `Certificado ${c.numero}/${c.ano} (aguardando emissão)`}
        ${emitido ? '<span style="color:#198754;"> ✓ Emitido</span>' : '<span style="color:#fd7e14;"> ⏳ Pendente</span>'}
      </h3>

      <h5 style="margin:20px 0 10px;">Dados do Relatório de origem</h5>
      ...
      <h5 style="margin:24px 0 10px;">Emissão</h5>
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
        <div>
